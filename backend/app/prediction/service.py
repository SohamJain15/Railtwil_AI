...
class PredictionService:
    ...
    def train(self, episodes=20, seed=2026):
        ...
        eta = XGBRegressor(n_estimators=40, max_depth=3, learning_rate=.08, random_state=seed, n_jobs=1); eta.fit(x.iloc[train_idx], frame.iloc[train_idx]["eta_target"]); pred = eta.predict(x.iloc[test_idx]); target=frame.iloc[test_idx]["eta_target"]; baseline=x.iloc[test_idx]["scheduled_remaining_seconds"]; metrics["eta"] = {"mae":float(mean_absolute_error(target,pred)),"rmse":float(mean_squared_error(target,pred)**.5),"baseline_mae":float(mean_absolute_error(target,baseline)),"baseline_rmse":float(mean_squared_error(target,baseline)**.5)}
        delay = XGBRegressor(n_estimators=40, max_depth=3, learning_rate=.08, random_state=seed, n_jobs=1); delay.fit(x.iloc[train_idx], frame.iloc[train_idx]["delay_target"]); pred = delay.predict(x.iloc[test_idx]); target=frame.iloc[test_idx]["delay_target"]; baseline=x.iloc[test_idx]["current_delay_seconds"]; metrics["delay"] = {"mae":float(mean_absolute_error(target,pred)),"rmse":float(mean_squared_error(target,pred)**.5),"baseline_mae":float(mean_absolute_error(target,baseline)),"baseline_rmse":float(mean_squared_error(target,baseline)**.5)}
        conflict = XGBClassifier(n_estimators=40, max_depth=3, learning_rate=.08, random_state=seed, n_jobs=1, eval_metric="logloss"); conflict.fit(x.iloc[train_idx], frame.iloc[train_idx]["conflict_target"]); prob = conflict.predict_proba(x.iloc[test_idx])[:,1]; labels = frame.iloc[test_idx]["conflict_target"]; precision, recall, f1, _ = precision_recall_fscore_support(labels, prob>.5, average="binary", zero_division=0); baseline_labels=(x.iloc[test_idx]["current_block_occupancy"]+x.iloc[test_idx]["junction_congestion"]>1.2); bp,br,bf,_=precision_recall_fscore_support(labels,baseline_labels,average="binary",zero_division=0); metrics["conflict"]={"precision":float(precision),"recall":float(recall),"f1":float(f1),"roc_auc":float(roc_auc_score(labels,prob)) if len(set(labels))>1 else None,"baseline_precision":float(bp),"baseline_recall":float(br),"baseline_f1":float(bf)}
